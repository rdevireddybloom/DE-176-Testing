...
def delete_all_objects(bucket_name, prefix=""):
    ...
    for obj in bucket.objects.filter(Prefix=prefix):
        ...
        if len(objects_to_delete) == 1000:
            try:
                ...
                logger.info(f"Deleted 1000 objects from {bucket_name} with prefix {prefix}")
            except Exception as e:
    ...
    if objects_to_delete:
        try:
            bucket.delete_objects(Delete={'Objects': objects_to_delete})
            logger.info(f"Deleted remaining {len(objects_to_delete)} objects from {bucket_name} with prefix {prefix}")
        except Exception as e:
            logger.error(f"Failed to delete objects: {e}")
...
if schedule_value == 'run_now':
    ...
    schedule_clause = " and coalesce(c.override_schedule, t.schedule) = '" + schedule_value + "'"
...
try:
    ...
    if schedule_value == 'error_rerun': 
        ...
        sql_to_run = "select distinct t.target_table, t.target_identifier, t.track_history, t.history_tracking_setup, \
            t.unique_record_fields, t.incr_where_clause, t.full_where_clause, t.run_order, t.data_transfer_target_id \
            from data_operations.data_transfer_target t \
                join data_operations.data_transfer_config c on t.data_transfer_target_id = c.data_transfer_target_id \
            where current_date between t.effective_start_date and t.effective_end_date \
                and current_date between c.effective_start_date and c.effective_end_date " + schedule_clause + " \
            order by t.run_order"
    ...
    for row in sql_results0:
        try:
            ...
            do_cr_cursor.execute("select max(currently_running::int)::boolean \
            ...
            if currently_running == True:
                counter = 0
                while counter < 5:
                    logger.info(f"Processing for: {target_table} running, letting finish. Interation: {counter}.  Sleeping: {sleep_time}.")
                    ...
                    if currently_running == False:
                        logger.info(f"Prior processing for: {target_table} has finished. Continue.")
                        break
                    
            # if still running close cursor, send alert and continue to next table (skipping this one)
            if currently_running == True:
                logger.info(f"Skipping table: {target_table}. ID: {data_transfer_target_id}.  Currently running.")
                do_cr_cursor.close()
                #  Send alert
                ...
                error_message_to_send = "During running of: dw_source_transfer glue job, Schedule: " + schedule_value + \
                            " table: " + target_table + " was skipped due to prior running process!!"
                ...
                do_cr_cursor.execute("update data_operations.data_transfer_target set currently_running = true \
                                        where data_transfer_target_id = " + str(data_transfer_target_id))
                do_conn.commit()
                do_cr_cursor.close()
            ...
            if schedule_value == 'error_rerun': 
                ...
                where current_date between t.effective_start_date and t.effective_end_date \
                    ...
                    and target_table = '" + target_table + "' " + schedule_clause + " \
                order by c.data_transfer_config_id asc"
            else:
                sql_to_run = "select data_transfer_config_id, data_transfer_type, \
                    source_host, source_db, source_schema, source_table, \
                    reference_id, overlap_treatment,force_full_run_update, transfer_sql, \
                    coalesce(override_incr_where_clause, ''), c.max_pk_val, c.pk_val_lookback_cnt \
                from data_operations.data_transfer_target t \
            ...
            for row in sql_results1:
                try:
                    ...
                    logger.info(f"Processing config ID: {schedule_value} -> {source_host}: {source_db}.{source_schema}.{source_table}")
                    
                    # Write initial record into the logging table
                    # Set process status = 'starting'
                    sql = "INSERT INTO data_operations.data_transfer_log(data_transfer_config_id, run_status, \
                                start_datetime) \
                         VALUES(" + str(data_transfer_config_id) + ",'starting',current_timestamp) RETURNING data_transfer_log_id;"
                    ...
                    rs_cursor.execute("truncate table " + landing_schema + "."+target_table)
                    rs_conn.commit()
                    
                    # Delete the S3 objects with the specified key
                    delete_all_objects(s3_data_bucket, s3_data_path + schedule_value + '_dw_src_xfer_')
                    ...
                    if data_transfer_type == "Incr":
                        ...
                        if "###MAX_PK_VAL###" in query:
                            ...
                            if max_pk_val <= -777777:
                                ...
                                do_cursor.execute("update data_operations.data_transfer_log \
                                    set run_status = 'max_pk_config_error', \
                                        data_prep_end_datetime = current_timestamp \
                                    where data_transfer_log_id = " + str(data_transfer_log_id))
                                do_conn.commit()
                            ...
                            query = query.replace("###MAX_PK_VAL###", str(max_pk_val_to_use))
                        else:
                    ...
                    for df in pd.read_sql(query, engine, chunksize=30000):
                        ...
                        df.to_csv("s3://"+s3_data_bucket+"/"+s3_data_path+schedule_value+"_dw_src_xfer_"+target_table+"_log_id" + \
                            str(data_transfer_log_id) + "_" + str(file_num_cnt) + ".txt", index=False, na_rep='NULL', \
                    ...
                    # Write updates to logging table
                    do_cursor.execute("update data_operations.data_transfer_log \
                        set transfer_count = " + str(transfer_count) + ", \
                    ...
                    rs_cursor.execute("copy " + landing_schema + "." + target_table + " \
                            from 's3://" + s3_data_bucket + "/" + s3_data_path + schedule_value + "_dw_src_xfer_" + \
                            target_table + "_log_id" + str(data_transfer_log_id) + "_' \
                            iam_role 'arn:aws:iam::511539536780:role/Redshift_for_copy' \
                            csv quote as '" + quote_char +"' delimiter '|' IGNOREHEADER 1 ROUNDEC NULL AS 'NULL'")
                            
                    # Pull count of records written to landing table
                    rs_cursor.execute("select count(*) from " + landing_schema + "." + target_table)
                    ...
                    if use_max_pk_val:
                        rs_cursor.execute("select " + str(data_transfer_config_id) + " as data_transfer_config_id, \
                            coalesce(max(" + pk_field + "), -999) from " + landing_schema + "." + target_table)
                    ...
                    # Write updates to logging table
                    do_cursor.execute("update data_operations.data_transfer_log \
                        set landed_count = " + str(landed_count) + ", \
                    ...
                    query = "insert into " + staging_schema + "." + target_table + " ( \
                    " + field_list + ",data_action_indicator) \
                        select n.*," + str(data_transfer_log_id) + ", " + md5_hash_sql + ", 'I'  \
                        from " + landing_schema + "." + target_table + " n \
                            left join " + integrated_schema +"." + target_table + " c on " + unique_record_join_cond + " \
                    ...
                    if overlap_treatment == "replace":
                        ...
                        query = "insert into " + staging_schema + "." + target_table + " ( \
                        " + field_list + ",data_action_indicator) \
                        select n.*," + str(data_transfer_log_id) + ", " + md5_hash_sql + ", 'U'  \
                        from " + landing_schema + "." + target_table + " n \
                            inner join " + integrated_schema +"." + target_table + " c on " + unique_record_join_cond
                    ...
                    if overlap_treatment == "update":
                        ...
                        from " + landing_schema + "." + target_table + " n \
                            inner join " + integrated_schema +"." + target_table + " c on " + unique_record_join_cond + \
                        " where coalesce(c.md5_hash, '') != " + md5_hash_sql
                    ...
                    rs_cursor.execute("select coalesce(sum(case when data_action_indicator = 'I' then 1 else 0 end), 0) as insert_count, \
                            ...
                            from " + staging_schema + "." + target_table + " \
                            where data_transfer_log_id = " + str(data_transfer_log_id))
                    ...
                    # Write updates to logging table
                    do_cursor.execute("update data_operations.data_transfer_log \
                        set insert_count = " + str(insert_count) + ", \
            ...
            do_cursor.execute("update data_operations.data_transfer_log l \
            ...
            if update_count_sum > 0 or insert_count_sum > 0:
                ...
                do_cursor.execute("update data_operations.data_transfer_log l \
                    ...
                    where l.data_transfer_config_id = c.data_transfer_config_id \
                        and run_status = 'staging_complete' \
                        and t.target_identifier = '" + target_identifier + "' \
                        and t.target_table = '" + target_table + "'")
                do_conn.commit()
                ...
                if track_history and history_tracking_setup:
                    ...
                    for row2 in result2:
                        history_tracking_field_list = history_tracking_field_list + 'n.' + row2.strip() + ','
                                
                    history_tracking_field_list = history_tracking_field_list.strip(',')
                        
                    query = "insert into " + history_schema +"." + target_table +"_history \
                                (" + field_list + ", record_version) \
                            with history_info as (select " + unique_record_fields + ", max(record_version) as max_version \
                                     from " + history_schema +"." + target_table +"_history \
                                     group by " + unique_record_fields + ") \
                            select " + history_tracking_field_list + ", \
                                coalesce(max_version,0) + 1 as record_version \
                            from " + staging_schema + "." + target_table + " n \
                                left join history_info c on " + unique_record_join_cond + " \
                ...
                if update_count_sum > 0:
                    query = "delete from " + integrated_schema +"." + target_table + " \
                        where dw_table_pk in ( \
                            select distinct c.dw_table_pk \
                                from " + integrated_schema +"." + target_table + " c \
                            join " + staging_schema + "." + target_table + " n on " + unique_record_join_cond + " \
                ...
                query = "insert into " + integrated_schema +"." + target_table + " ( \
                        " + field_list + ") \
                        select " + field_list + " from " + staging_schema + "." + target_table + " \
                ...
                for result_line in max_pk_apply_list:
                    ...
                    do_cursor.execute("update data_operations.data_transfer_config \
                        set max_pk_val = " + str(max_pk_val_to_upd) + "  \
                        where  data_transfer_config_id = " + str(dt_config_id))
                    
                    # Keeping max_pk_val updates for all schedules
                    do_cursor.execute("update data_operations.data_transfer_config c \
                        ...
                        and c2.data_transfer_config_id = " + str(dt_config_id) + "  \
            ...
            do_cursor.execute("update data_operations.data_transfer_target set currently_running = false \
                                where data_transfer_target_id = " + str(data_transfer_target_id))
            do_conn.commit()
    ...
    if data_prep_error_count > 0:
        ...
        error_message_to_send = "During running of: dw_source_transfer glue job, " + str(data_prep_error_count) + " data prep errors were encountered!!"
        msg_id = publish_message(sns_topic, error_message_to_send, error_email_subject)
        
    if finalizing_error_count > 0:
        # Send notification to SNS topic that errors were encountered
        error_email_subject = "DW Source Transfer (Finalizing) Errors Encountered!"
        error_message_to_send = "During running of: dw_source_transfer glue job, " + str(finalizing_error_count) + " data finalizing errors were encountered!!"
        msg_id = publish_message(sns_topic, error_message_to_send, error_email_subject)
