...
s3_data_path = "Focused_BI_Dashboard_Datasets/"
...
def move_s3_file(bucket_name, source_key, destination_key):
    ...
    try:
        ...
        logger.error(
            f"Error moving file from {source_key} to {destination_key} in bucket {bucket_name}: {e}"
        )
...
try:
    ...
    finalizing_sql = [
        "id::int, start_time::timestamp, completion_time::timestamp, email, name, date_of_delivery::date, \
        ...
        application_date::date, supervisor, agent_name, npn, complaint_type, allegation, \
    ...
    final_schema_to_load = "sandbox_tsaxen"
    ...
    for i in range(len(file_names)):
        ...
        process_s3_location = (
            "s3://" + s3_data_bucket + "/" + s3_data_path + file_names[i]
        )
        print(process_s3_location)
        print(sheet_names[i])
        ...
        rs_cursor.execute(
            "truncate table " + initial_schema_to_load + "." + tables_to_load[i]
        )
        rs_conn.commit()
        ...
        rs_cursor.execute(
            ...
            + "_focBI_' \
                            ...
                            csv quote as '"
            + quote_char
            + "' delimiter '|' IGNOREHEADER 0 ROUNDEC NULL AS 'NULL'"
        )
        rs_conn.commit()
        ...
        rs_cursor.execute(
            "truncate table " + final_schema_to_load + "." + tables_to_load[i]
        )
        rs_cursor.execute(
            "insert into "
            + final_schema_to_load
            + "."
            + tables_to_load[i]
            + " \
            select "
        ...
        response = s3_client.delete_object(
            Bucket=s3_data_bucket, Key=file_key_to_delete
        )

        # Copy files processed to an archive folder with a date added in name
        source_key = s3_data_path + file_names[i]
        destination_key = (
            s3_data_path + "Archive/date_" + file_date + "_" + file_names[i]
        )

        move_s3_file(s3_data_bucket, source_key, destination_key)
