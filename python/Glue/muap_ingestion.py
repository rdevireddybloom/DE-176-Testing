...
def generate_sns_notification(error_context, params = None):
    ...
    try:
        ...
        params.setdefault('timestamp', datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S'))
        params.setdefault('job_name', job_name) 

        #templates for SNS messages
        templates = { "s3_upload_failed" : {
        ...
        template  = templates.get(error_context , {
        ...
        logger.info(f"Successfully published SNS notification for {error_context}. Message ID: {messageID}")

        #logging to cloudwatch as well
        logger.error(f"Error notification sent for {error_context}:\n{final_message} \nMessage ID: {messageID}")
...
def upload_to_s3(url, bucket, prefix):
   ...
   logger.info(f"Starting download of CSV from {url} and upload to S3 bucket: {bucket}")
   ...
   try:
       ...
       generate_sns_notification("s3_upload_failed", {
               "sns_topic": sns_topic,
               "url": url,
              "bucket": bucket,
              "prefix": prefix,
...
def validate_schema(s3_uri, conn):
    ...
    try:
        ...
        #Publish error message to SNS topic
        generate_sns_notification("schema_validation_failed", {
            "sns_topic": sns_topic,
            "s3_uri": s3_uri,
...
def load_to_redshift(conn, s3_uri):
    ...
    try:
        ...
        cursor.execute("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'staging_mua_p_data')")
        stg_table_exists = cursor.fetchone()[0]
        
        # Check if the final table exist
        cursor.execute("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'mua_p_data')")
        ...
        # Publish error message to SNS topic
        generate_sns_notification("redshift_load_failed", {
            "sns_topic": sns_topic,
            "s3_uri": s3_uri,
            "table_name": "mua_p_data",
            "error": str(e)
...
source_url =  args['source_url'] #'https://data.hrsa.gov//DataDownload/DD_Files/MUA_DET.csv'
