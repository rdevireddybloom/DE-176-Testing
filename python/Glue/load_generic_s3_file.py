...
try:
    ...
    if file_type == "xlsx":
        ...
        df = pd.read_excel(
            process_s3_location, sheet_name=sheet_name_if_xlsx, dtype=str
        )
    ...
    if file_type == "csv":
        ...
        if delimiter_if_csv == ",":
            df = pd.read_csv(
                process_s3_location, sep=",", encoding_errors="replace", dtype=str
            )
        if delimiter_if_csv == "|":
            df = pd.read_csv(
                process_s3_location, sep="|", encoding_errors="replace", dtype=str
    ...
    if write_to_db == "redshift":
        ...
        url = (
            "postgresql://"
            + rs_secret_json["username"]
            + ":%s@"
            + rs_secret_json["host"]
            + ":"
            + str(rs_secret_json["port"])
            + "/"
            + rs_secret_json["dbname"]
        )
